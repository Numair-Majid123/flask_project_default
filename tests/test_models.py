...
def test_todo_creation(client):
    with app.app_context():
        ...
        todo = ToDo(title='Test Todo', description='This is a test', due_date=datetime.now() + timedelta(days=1), user_id=user.id)
        db.session.add(todo)
        db.session.commit()
...
def test_todo_validation(client):
    with app.app_context():
        ...
        with pytest.raises(ValueError):
            ToDo(title='Test', description='Test', due_date=datetime.now() - timedelta(days=1), user_id=user.id)
